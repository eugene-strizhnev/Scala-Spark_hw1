...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf()
    .setMaster("local[2]")
    .setAppName("WikipediaAnalyzer")
  val sc: SparkContext = new SparkContext(conf)
  // Hint: use a combination of `sc.textFile`, `WikipediaData.filePath` and `WikipediaData.parse`
  val wikiRdd: RDD[WikipediaArticle] = sc.textFile(WikipediaData.filePath, 4).map(WikipediaData.parse)
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    (count, article) => {
      if (article.text.split(" ").contains(lang)) count + 1 else count
    }, _ + _)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap({ x =>
      val split = x.text.split(" ")
      langs.collect {
        case lang: String if split.contains(lang) => (lang, x)
      }
    }).groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd.flatMap({
    x =>
      val split = x.text.split(" ")
      langs.collect {
        case lang if split.contains(lang) => (lang, 1)
      }
